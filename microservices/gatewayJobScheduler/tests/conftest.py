...
os.environ.update({
    'KC_SERVER_URL': 'https://test-keycloak-server',
    'KC_REALM': 'test-realm',
...
def mock_keycloak_clients():
    """Provide mock Keycloak client for all tests"""
    yield {
        'admin_client': mock_keycloak_admin
    }

@pytest.fixture(autouse=True)
def reset_keycloak_mock():
    """Reset mock state between tests"""
    mock_keycloak_admin.reset_mock()
    yield
