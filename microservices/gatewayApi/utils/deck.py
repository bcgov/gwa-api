...
def deck_cmd_sync_diff(deck_cli, cmd, select_tag, state):
    if deck_cli == "deck" and cmd == "dump":
        return [ deck_cli, "gateway", cmd, "--config", "/tmp/deck.yaml", "--skip-consumers", "--select-tag", select_tag]
    elif deck_cli == "deck" or deck_cli.startswith("deck_kong3_"):
