...
def validate_base_entities(yaml, ns_attributes):
    traversables = ['_format_version', '_plugin_configs', 'services', 'upstreams', 'certificates', 'consumers', 'key_sets', 'keys']
...
def traverse(source, errors, yaml, required_tag, qualifiers):
    ...
    if source == "":
        traversables = ['services', 'upstreams', 'consumers', 'certificates', 'consumers', 'key_sets', 'keys', 'ca_certificates']
    else:
...
def traverse_tags_transform(yaml, namespace, required_tag):
    object_count = 0
    log = app.logger
    traversables = ['services', 'routes', 'plugins', 'upstreams', 'certificates', 'consumers', 'key_sets', 'keys', 'ca_certificates']
    for k in yaml:
        if k in traversables:
            for item in yaml[k]:
                if 'tags' in item:
